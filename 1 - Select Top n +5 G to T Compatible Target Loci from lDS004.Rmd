---
title: "Select Top *n* +5 G to T Compatible Target Loci from lDS_004"
...
selected <- lds004 %>% select(pegRNA_ID, cell_line, Gene, Guide, PBS, RT_template, Barcode,
                              ...
                              PBS_length, RT_length, GC_percent_PBS)
...
wide_avg <- wide_avg %>% select(pegID = pegID, 
...
nrow(diff)
## remove any duplicate entries under Target_Locus
diff <- diff %>%  filter(Edited_nt == 'T')
nrow(diff)
...
```{r export}
# export files
write.csv(top, 'MMR-responsive_+5GtoT_target_candidates.csv', row.names=TRUE)
```

## 3.2 Export Ann's joined datasets, with PEmax vs MLH1KO difference

```{r export diff}
write.csv(diff, 'lDS004_joined_+5GtoT_mean.csv', row.names=TRUE)